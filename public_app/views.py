...
def history(request):
    recent_donations = CustomUser.objects.filter(
        role='donor',
        is_verified=True
    ).order_by('-date_joined')[:10]

    recent_stocks = BloodStock.objects.all().order_by('-created_at')[:10]

    context = {
        'recent_donors': recent_donations,
        'recent_stocks': recent_stocks,
        'total_donors': recent_donations.count(),
        'total_stocks': recent_stocks.count(),
    }

    return render(request, 'public_app/history.html', context)